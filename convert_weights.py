...
def main():
    ...
    parser.add_argument(
        "--checkpoint_folder", type=str
    )
    args = parser.parse_args()
    if not args.checkpoint_folder:
        args.checkpoint_folder = f'llama-2-{args.model_size.lower()}-chat'

    write_model(
        input_base_path=os.path.join(args.input_dir, args.checkpoint_folder),
